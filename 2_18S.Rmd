...
set.seed(123)
numperm=999 #Permutation tests
numsam=200 #TITAN2 bootstrap samples
...
otu_decontam=read_csv(
  file.path(
    ...
    "OTU_table_taxonomy_decontaminated_unfiltered_18S.csv"
  )
)
...
otu_clean=otu_unfiltered %>%
  ...
  ) ##filter non-metazoans and unidentified micro-algae
...
write_csv(
  ...
  file.path(paths$data_processed, "OTU_reads_18S.csv")
)
...
#write_csv(as.data.frame(otu_hellinger) %>% rownames_to_column("OTU_ID"), file.path(paths$results_tables, "OTU_hellinger_18S.csv"))
...
write_csv(as.data.frame(otu_index) %>% rownames_to_column("OTU_ID"), file.path(paths$results_tables, "OTU_eDNAindex_18S.csv"))
```
...
#Convert OTU table to list format required by iNEXT
#Each element is a sample with OTU abundances (only OTUs with >0 reads)
otu_list_samples=apply(
  otu_matrix,
  2,  #Apply over columns (samples)
  function(x) {
    x=x[!is.na(x)]  #Remove NAs if any
    x=x[x > 0]      #Keep only OTUs with counts > 0
    return(x)
  }
)
...
###########################################################################
inext_out=iNEXT(otu_list_samples, q=c(0, 1, 2), datatype="abundance", se=FALSE)
saveRDS(inext_out, file=file.path(paths$results_rds,"2iNEXT_hill_18S.rds"))
###########################################################################
inext_out=readRDS(file=file.path(paths$results_rds, "2iNEXT_hill_18S.rds"))

#Get coverage information
coverage_info=inext_out$DataInfo

#Find the minimum coverage among samples (or choose a specific target)
target_coverage=min(coverage_info$SC, na.rm=TRUE)
cat("Target coverage:", target_coverage, "\n")

####Coverage-Standardization ####################################
coverage_std=estimateD(
  otu_list_samples,
  q=c(0, 1, 2),
  datatype="abundance",
  base="coverage",  #Standardize by coverage
 level=target_coverage, conf=0.95)

hill_df_cov=coverage_std %>%
  ...
  dplyr::rename(SAMPLE_ID=Assemblage) %>%
  dplyr::rename(Richness_cov=Hill_q0) %>%
  dplyr::rename(Shannon_cov=Hill_q1) %>%
  dplyr::rename(Simpson_cov=Hill_q2)
#############################################################

#### Extrapolated Richness #################################
hill_df_ext=inext_out$AsyEst %>%
 dplyr::select(Assemblage, Diversity, Estimator) %>%
 tidyr::pivot_wider(
   names_from=Diversity,
   values_from=Estimator) %>%
 dplyr::rename(SAMPLE_ID=Assemblage) %>%
 dplyr::rename(Richness_ext=`Species richness`) %>%
 dplyr::rename(Shannon_ext=`Shannon diversity`) %>%
 dplyr::rename(Simpson_ext=`Simpson diversity`)
###########################################################

#### Observed Richness ####################################
hill_df_obs=inext_out$AsyEst %>%
 dplyr::select(Assemblage, Diversity, Observed) %>%
 tidyr::pivot_wider(
   names_from=Diversity,
...
hill_df <- hill_df_obs %>%
  dplyr::full_join(hill_df_ext, by = "SAMPLE_ID") %>%
  dplyr::full_join(hill_df_cov, by = "SAMPLE_ID")

#Merge with metadata
alpha_div=hill_df %>%
  left_join(sample_metadata, by="SAMPLE_ID")
alpha_div$MARKER="18S"

write_csv(alpha_div,file.path(paths$results_tables, "alpha_div_18S_iNEXT.csv"))

p1=ggplot(alpha_div, aes(x=SITE, y=Richness, fill=SITE)) +
...
p2=ggplot(alpha_div, aes(x=SITE, y=Shannon, fill=SITE)) +
  geom_violin(alpha=0.5, scale="width") +
  geom_point(position=position_jitter(width=0.2), size=0.8) +
  geom_boxplot(width=0.1, outlier.shape=NA) +
...
p3=ggplot(alpha_div, aes(x=SITE, y=Simpson, fill=SITE)) +
  geom_violin(alpha=0.5, scale="width") +
  geom_point(position=position_jitter(width=0.2), size=0.8) +
  geom_boxplot(width=0.1, outlier.shape=NA) +
...
##---- NMDS ----
#####################################################################
nmds=metaMDS(dist_matrix, k=3, trymax=500, trace=T, weakties=TRUE, maxit=numperm, pc=TRUE, zerodist="add", autotransform=F)
saveRDS(nmds, file.path(paths$results_rds, "nmds_18S.rds"))
#####################################################################
nmds=readRDS(file.path(paths$results_rds, "nmds_18S.rds"))
...
write.csv(nmds_scores, file=file.path(paths$results_tables, "NMDS_data_18S.csv"), row.names=F)

#Choose which abiotic predictors to fit
...
write.csv(scores_env, file=file.path(paths$results_tables, "envfit_18S.csv"), row.names=F)
...
nmdsplot=ggplot(
  ...
  theme_bw() +
  labs(
    title="18S NMDS (Jaccard, eDNA index)",
...
anova(sitestep_dispersion) ##p=0.002, df=67, F=1.7=ANOSIM
...
anosim.sitestep #p=0.001, R=0.90

##---- Variance partitioning ---
varpart.out=varpart(dist_matrix, ~ SITE, ~ STEP, data=sample_metadata)
summary(varpart.out) #X1 site=0.08, X2 step=0.004 unique effects

#Test unique contribution of SITE
anova(dbrda(dist_matrix ~ SITE + Condition(STEP), data=sample_metadata), permutations=numperm) #p=0.001, df=3, F=6.51

#Test unique contribution of STEP
anova(dbrda(dist_matrix ~ STEP + Condition(SITE), data=sample_metadata), permutations=numperm) #p=0.001, df=1, F=1.89
```
...
saveRDS(rf_nmds, file.path(paths$results_rds, "RFspatialmodel_18S.rds"))
print(rf_nmds) #mtry=2, accuracy=0.97, kappa=0.96
```
...
saveRDS(rf_nmds_step, file.path(paths$results_rds, "RFtemporalmodel_18S.rds"))
print(rf_nmds_step) #mtry=3, accuracy=0.20, kappa=0.15
```
...
saveRDS(rf_nmds_int, file.path(paths$results_rds, "RFinteractionmodel_18S.rds"))
print(rf_nmds_int) #mtry=2, accuracy=0.24, kappa=0.23
```
...
```{r}
community_index=t(otu_index)
community_index=community_index[
  rownames(community_index) %in% sample_metadata$SAMPLE_ID,
]

community_table_index=as.data.frame(t(community_index))

community_table_index=community_table_index %>%
  rownames_to_column("OTU_ID")

community_df_index=community_table_index %>%
  left_join(taxa, by="OTU_ID")

community.data=community_df_index
rownames(community_df_index)=community_df_index$OTU_ID

data.long=community.data %>% 
  pivot_longer(cols=2:(ncol(community.data)-9), 
               names_to="SAMPLE_ID", 
               values_to="value") %>%
  dplyr::select(OTU_ID, phylum, class, order, family, genus, species, SAMPLE_ID, value)

data.long.narm=filter(data.long, value>0)

data.long.both=merge(data.long.narm, metadata, by="SAMPLE_ID")
data.long.both$value=as.numeric(data.long.both$value)
data.long.both$SITE=as.factor(data.long.both$SITE)
data.long.both$STEP=as.factor(data.long.both$STEP)

write_csv(data.long.both, file.path(paths$data_processed, "eDNA_data_long_transformed_18S.csv"))
```
...
saveRDS(sync.kaneohe, file.path(paths$results_rds, "sync_kaneohe_18S.rds"))
########################################################################
sync.kaneohe=readRDS(file.path(paths$results_rds, "sync_kaneohe_18S.rds"))
...
saveRDS(sync.lanikai, file.path(paths$results_rds, "sync_lanikai_18S.rds"))
#######################################################################
sync.lanikai=readRDS(file.path(paths$results_rds, "sync_lanikai_18S.rds"))
...
saveRDS(sync.maunalua, file.path(paths$results_rds, "sync_maunalua_18S.rds"))
########################################################
sync.maunalua=readRDS(file.path(paths$results_rds, "sync_maunalua_18S.rds"))
...
saveRDS(sync.waimanalo, file.path(paths$results_rds, "sync_waimanalo_18S.rds"))
########################################################
sync.waimanalo=readRDS(file.path(paths$results_rds, "sync_waimanalo_18S.rds"))
...
synchrony.data$Marker="18S"

write.csv(synchrony.data, file=file.path(paths$results_tables, "TABLE_S9_synchrony_18S.csv"), row.names=F)
#write.csv(synchrony.data, file=file.path(paths$results_tables, "TABLE_S9_synchrony_18S_motu"), row.names=F)
```
...
write_csv(
  stability,
  file.path(paths$results_tables, "TABLE_S10_stability_18S.csv")
)
```
...
write_csv(turnover_site, file.path(paths$results_tables, "turnover_18S.csv"))
...
saveRDS(titan_result, file.path(paths$results_rds, "titan_18S.Rdata"))
##################################################################
titan_result=readRDS(file.path(paths$results_rds, "titan_18S.Rdata"))
...
ggsave(
  filename=file.path(paths$results_figs, "FIGURE_S6_18S.pdf"),
  plot    =titanplot,
  width   =8,
  height  =4,
  units   ="in",
...
saveRDS(zeta, file.path(paths$results_rds, "zetadecline_scaled_18S.Rdata"))

#Load and plot
zeta=readRDS(file.path(paths$results_rds, "zetadecline_scaled_18S.Rdata"))
Plot.zeta.decline(zeta)
zeta$aic #exp=-33.74, pl=-77.16

tib.decay=zeta[1:4] %>% as.tibble() %>% mutate(Marker="18S")
write_csv(tib.decay, file.path(paths$results_tables, "zeta_decay_scaled_marker18S.csv"))

tib.ratio=zeta[5] %>% as.tibble() %>% mutate(zeta.order=1:n(), Marker="18S")
write_csv(tib.ratio, file.path(paths$results_tables, "zeta_ratio_scaled_marker18S.csv"))
```
...
for (site in names(sites)) {
  ...
  saveRDS(
    zeta.mc.site,
    file.path(paths$results_rds, paste0("zetadecline_", site, "_scaled_18S.Rdata"))
  )
...
tib.zeta=imap_dfr(zeta.site, ~ {
  ...
  .x[1:4] %>%                 #select first 4 components
    ...
    mutate(Site=.y, Marker="18S")          #add site column
})
write_csv(tib.zeta, file.path(paths$results_tables, "zeta_decay_scaled_18S.csv"))
...
write_csv(zeta.ratio.tib, file.path(paths$results_tables, "zeta_ratio_scaled_18S.csv"))
...
tib.zeta.aic=imap_dfr(zeta.site, ~ {
  ...
  group_by(Site) %>%
  mutate(
    delta_AIC=AIC - min(AIC), Marker="18S"
  ) %>%
  ungroup()

write_csv(tib.zeta.aic, file.path(paths$results_tables, "zeta_aic_18S.csv"))
```
...
taxon_counts_site$Marker="18S"
write_csv(taxon_counts_site, file.path(paths$results_tables, "coreMOTUs_site_18S.csv"))
```
