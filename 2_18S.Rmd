...
## ---- run or load iNEXT ----
inext_file=file.path(paths$results_rds, "iNEXT_sample_18S.rds")
...
#write_csv(as.data.frame(otu_coverage) %>% rownames_to_column("OTU_ID"), file.path(paths$data_processed, "OTU_coverage_18S.csv"))

## ---- transformations ----

# Hellinger
otu_hellinger=decostand(t(otu_coverage), method="hellinger")
...
# Presence–absence
otu_pa=decostand(t(otu_coverage), method="pa")
otu_pa=t(otu_pa)

# eDNA index
relative_abundance=sweep(otu_coverage, 2, colSums(otu_coverage), "/")
...
nmdsplot=ggplot(
  ...
  theme_bw() +
  labs(
    title="18S NMDS (Bray–Curtis, eDNA index)",
...
anova(sitestep_dispersion) ##p=0.0002, df=67, F=2.05 = ANOSIM
...
print(rf_nmds) # mtry = 2, accuracy = 0.956, kappa = 0.942
```
...
print(rf_nmds_step) # mtry = 2, accuracy = 0.253, kappa = 0.204
```
...
print(rf_nmds_int) # mtry = 2, accuracy = 0.219, kappa = 0.208
```
...
write_csv(
  stability,
  file.path(paths$results_tables, "TABLE_S9_stability_18S.csv")
)

write_csv(otu_long, file.path(paths$data_processed, "data_long_18S.csv"))
```
...
ggsave(
  filename = file.path(paths$results_figs, "FIGURE_S4_18S.pdf"),
  plot     = titanplot,
  width    = 8,
  height   = 4,
  units    = "in",
