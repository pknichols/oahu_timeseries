...
saveRDS(inext_out, file=file.path(paths$results_rds,"2iNEXT_hill_COI.rds"))
###########################################################################
inext_out=readRDS(file=file.path(paths$results_rds, "2iNEXT_hill_COI.rds"))
...
alpha_div$MARKER="COI"

write_csv(alpha_div,file.path(paths$results_tables, "alpha_div_COI_iNEXT.csv"))
...
nmdsplot=ggplot(
  ...
  theme_bw() +
  labs(
    title="COI NMDS (Jaccard, eDNA index)",
...
print(rf_nmds) #mtry=2, accuracy=0.99, kappa=0.99
```
...
print(rf_nmds_step) #mtry=2, accuracy=0.27, kappa=0.23
```
...
print(rf_nmds_int) #mtry=3, accuracy=0.29, kappa=0.28
```
...
write_csv(data.long.both, file.path(paths$data_processed, "eDNA_data_long_transformed_COI.csv"))
```
...
write_csv(
  stability,
  file.path(paths$results_tables, "TABLE_S10_stability_COI.csv")
)
```
...
ggsave(
  filename=file.path(paths$results_figs, "FIGURE_S6_COI.pdf"),
  plot    =titanplot,
  width   =8,
  height  =4,
  units   ="in",
...
for (site in names(sites)) {
  ...
  saveRDS(
    zeta.mc.site,
    file.path(paths$results_rds, paste0("zetadecline_", site, "_scaled_COI.Rdata"))
  )
