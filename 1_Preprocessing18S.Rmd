...
# INSECT repo https://github.com/shaunpwilkinson/insect ##download 18S and COI INSECT classifier.rds
...
```{r}
# Classifier
classifier = readRDS(file.path(paths$data_raw, "classifier_18S.rds"))

# Sequences
data = readFASTA(file.path(paths$data_raw, "data_rep_set_18S.fasta"))

# OTU table
otutab = read.delim(file.path(paths$data_raw, "dataOTU_18S.txt"), sep = "\t", header = TRUE, row.names = 1)

# Match list
matchlist = read.delim(file.path(paths$data_raw, "match_list_18S.txt"), sep = "\t", header = FALSE)
...
write.csv(insect_df, file.path(paths$data_processed, "insect085taxonomy_18S.csv"), row.names = FALSE)

## ---- merge_otu_taxonomy ----
insect_df = read.csv(file.path(paths$data_processed, "insect085taxonomy_18S.csv"), header = TRUE)
...
write.csv(microdecon_removed, file.path(paths$results_tables, "TABLE_S3_microdecon_removed_18S.csv"), row.names = FALSE)

# Merge with taxonomy
table_unfiltered = merge(decon_result$decon.table, otu_info, by = "OTU_ID")
table_unfiltered$Mean.blank = NULL
write.csv(table_unfiltered, file.path(paths$results_tables, "OTU_table_taxonomy_decontaminated_unfiltered_18S.csv"), row.names = FALSE)
```